...
const enterNode = selection => {
  selection.select("circle").attr("r", d => {
    return Math.sqrt(d.sanctioncurrent / Math.PI) / 180;
  });
  // .style("fill", "#000000")
  // .style("opacity", ".7")
...
const updateNode = selection => {
  ...
  selection.select("circle").attr("cx", d => d.x);
  // .attr("cy", d => d.y)
...
class FForce2 extends React.Component {
  ...
  componentDidMount() {
    ...
    // .force("yPos", d3.forceY(d => scale(d.rateOfChange)).strength(0.1))
    ...
    force.on("tick", () => {
      var q = d3.quadtree(this.props.data.nodes),
        i = 0,
        n = this.props.data.nodes.length;

      while (++i < n) {
        q.visit(collide(this.props.data.nodes[i]));
      }
  ...
  render() {
    ...
    var nodes = this.props.data.nodes.map((node, index) => {
      ...
      if (node.rateOfChange < 0) {
        ...
        node.colorCode = 1;
      } else if (node.rateOfChange > 0.25 && node.rateOfChange < 0.5) {
        node.colorCode = 2;
      } else if (node.rateOfChange > 0.5 && node.rateOfChange < 0.75) {
        node.colorCode = 3;
    ...
    return (
      ...
      >
        <Axis
          {...axisPropsFromTickScale(scaleAxis, 5)}
          style={{ orient: LEFT }}
        />
        <g>{nodes}</g>
        <g
        ...
        >
          <g id="Group">
            ...
            <text id="Line-2" font-size="12" fill="#4A4A4A">
              <tspan x="10" y="37">
                Sanction This Year: {sanctioncurrent}
              </tspan>
            </text>
            <text id="Line-3" font-size="12" fill="#4A4A4A">
              <tspan x="10" y="55">
                Sanction Last Year: {sanctionprevious}
              </tspan>
            </text>
...
class Node extends React.Component {
  ...
  render() {
    return (
      <g className="node">
        <circle className="node" cy={this.props.cy} ref="dragMe" />
        {
