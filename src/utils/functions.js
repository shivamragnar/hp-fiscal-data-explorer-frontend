...
export const prepQueryStringForFiltersApi = (activeFilters) => {
  ...
  activeFilterVals.map((val, i) => {
    let tempVal0 = val.map((item) => {
      return item.split("-")[0];
    });
    let tempVal1 = val.map((item) => {
      return item.split("-")[1];
    });
    ...
    if (activeFilterKeys[i].split("-")[1] !== undefined) {
      stringForApi +=
        "&" + activeFilterKeys[i].split("-")[1] + '="' + tempVal1 + '"';
    }
...
export const calcXTickVals = (month_week, districtwiseVals) => {
  ...
  let corresWeekNum =
    month_week !== "month" &&
    districtwiseVals[0].map((d, i) => {
      if (i !== 0 && districtwiseVals[0][i - 1][0] > d[0]) {
        //if year has changed
...
export const calcXTickFormats = (
  ...
  let corresWeekDateRange =
    ...
    districtwiseVals[0].map((d, i) => {
      ...
      let xTickString =
        moment(yearForMoment.split("-")[0])
          .add(d[0], "weeks")
          .startOf("week")
          .format(
            switchYearGate === true ? "DD MMM YY" : "DD MMM"
          ) /*gets the sunday of the week*/ +
        " - " +
        moment(yearForMoment.split("-")[0])
          .add(d[0], "weeks")
          .endOf("week")
          .format("DD MMM YY"); /*gets the saturday of the week*/
...
export const getWeekwiseDates = (
  ...
  for (var i = fromMonthIndex; i <= toMonthIndex + toMonthIndexAddOn; i++) {
    const iMod = i % 12;
    const jsDateFrom = new Date(
      years[
        Math.floor(((i % fromMonthIndex) + fromMonthIndex) / 12) + fromYearIndex
      ] +
        `-0${iMod + 1}-` +
        dateFrom.split("-")[2]
    );
    ...
    if (iMod === toMonthIndex) {
      //if this is the last month only then add the end of month date
    ...
    const weekwiseDatesOfMonth = weekwiseDaysOfMonth.map((day) => {
      return (
        day +
        " " +
        months[((i % fromMonthIndex) + fromMonthIndex) % 12] +
        " " +
        years_short[
          Math.floor(((i % fromMonthIndex) + fromMonthIndex) / 12) +
            fromYearIndex
        ]
      );
    });
  ...
  return {
    date_for_tick: allWeekWiseDays,
    date_for_x_axis: totWeekWiseDayNums,
  };
};
...
//6
export const onDateRangeChange = (newDateRange) => {
  //the month number-range is coming in as 1 - 12
  ...
  Object.keys(obj).map((obj_key) => {
    if (obj[obj_key]) {
      ...
      if (activeFilters.filters.hasOwnProperty(filterOrderRef[idx]) === true) {
        if (
          activeFilters.filters[filterOrderRef[idx]] === obj_key.split("-")[0]
        ) {
          recursFilterFind(
            obj[obj_key],
            query,
...
export const recursFilterFetch = (allFiltersData, obj, idx) => {
  Object.keys(obj).map((obj_key) => {
    if (obj[obj_key]) {
      ...
      if (
        allFiltersData[idx].val.some((item) => item.id === obj_key) !== true
      ) {
        const filterOption = {
...
//10
//find all the places where the selected filter exists.
export const recursFilterFind2 = (
  obj,
  query,
  results,
  idx,
...
//11
export const resetFiltersToAllFilterHeads = (
...
export const createBudgetCodeString = (
  activeFilterVals,
  activeFilterKeys,
  filterOrderRef,
  range
) => {
  //the range array is the indices between which we wanna
  ...
  const budgetCodeStringArray_clean = budgetCodeStringArray.filter(
    (elem) => elem !== undefined
  );
...
export const createObjForPayload = (activeFilterVals, activeFilterKeys) => {
  ...
  activeFilterVals.map((val, i) => {
    ...
    if (activeFilterKeys[i].split("-")[1] !== undefined) {
      //this is a check to see if our filter is if of the CODE_NAME format or simply a CODE format
